...
export function Topbar() {
  ...
  return (
    <header className="shrink-0 flex items-center justify-between h-14 px-4 md:px-6 bg-slate-950 border-b border-slate-800 gap-4">

      {/* Left — page title + breadcrumb */}
      <div className="flex flex-col justify-center min-w-0">
        <span className="text-[15px] font-bold text-slate-100 leading-tight truncate">
          {meta.title}
        </span>
        <span className="hidden sm:block text-[11px] text-slate-500 leading-tight truncate">
          {meta.breadcrumb}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div
          className={cn(
            ...
            searchFocused
              ? 'w-52 border-indigo-500 bg-slate-800 ring-1 ring-indigo-500/50'
              : 'w-36 border-slate-700 bg-slate-900 text-slate-400',
          )}
        >
          ...
          <input
            ...
            className="bg-transparent outline-none w-full text-slate-200 placeholder-slate-500 text-xs"
          />
        </div>

        {/* Bell */}
        <button
          className="relative flex items-center justify-center w-8 h-8 rounded-lg border border-slate-700 bg-slate-900 text-slate-400 hover:text-slate-200 hover:border-slate-600 transition-colors"
          aria-label="Notifications"
        >
          <Bell className="w-4 h-4" />
          {/* Unread dot — will be driven by notification state in Sprint 14 */}
